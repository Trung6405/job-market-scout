...
- 2026-07-23: Refined gap semantics so only technical skills are eligible to be gaps (see `docs/agent/specs/gap-kind-classification/spec.md`). Each extracted requirement now carries a closed-vocabulary `kind` (`skill`/`qualification`/`experience`/`soft_skill`); only `skill`-kind requirements are matched against the profile's `tech_stack` and can become gaps. Non-skill requirements (degrees, experience thresholds, soft skills) previously matched a bare tech-stack token, never matched, and always surfaced as false gaps — e.g. "a STEM degree in CS" flagged missing for a profile holding a CS degree. They are now shown as display-only context ("Role also asks for") with no met/unmet mark, excluded from the gap list and from must-have coverage counts. `evaluate_requirements` returns non-skill checks with `met=True` and their kind; `get_run_details` selects gaps on `kind == "skill" and not met`; `listing_gaps` gained a `kind` column (default `'skill'`, so legacy rows read as skill). This supersedes the earlier "checklist is scoped to `tech_stack` skills only" note above — the checklist is still skill-only, but non-skill requirements are no longer silently dropped or mis-flagged; they render as context.
