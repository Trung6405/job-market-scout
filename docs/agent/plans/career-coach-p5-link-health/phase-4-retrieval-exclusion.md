...
- **Gate:** none
- **Steps:**
  - [x] Write failing test: two embedded resources both match a skill's
  ...
  - [x] Implement: add `AND dead_since IS NULL` to the lateral subquery's
  ...
  - [x] Commit: `feat(coach): exclude dead resources from retrieval`
...
- **Gate:** none
- **Steps:**
  - [x] Write failing test: a resource with `last_verified` NULL,
  ...
  - [x] Verify it fails or passes as written — if it passes immediately,
  ...
  - [x] Implement: no change expected; if the test fails, the Task 1 condition
        was written too broadly (e.g. keying off `consecutive_failures` instead
        of `dead_since`) — fix it there
  - [x] Verify it passes (`pytest tests/test_coach_retrieval_db.py -q`)
  - [x] Commit: `test(coach): guard that unchecked resources stay retrievable`
...
- **Gate:** none
- **Steps:**
  - [x] Write failing test: a resource that `record_link_check` marks dead via
  ...
  - [x] Implement: no production change expected; this test wires Phase 3's
        writer to Phase 4's reader
  - [x] Verify it passes (`pytest tests/test_coach_link_health_db.py -q`)
  - [x] Commit: `test(coach): cover dead-to-retrieval round trip`
...
- [x] The retriever's own behaviour is unchanged for healthy corpora:
      `pytest tests/test_coach_retriever.py -q`
- [x] The grounded-tip stage — a live consumer of this read path since the P3
      merge — is unaffected:
...
No deviations. The grounded-tip stage's own suite (`test_coach_tips.py`,
`test_coach_tips_db.py`) stayed green, confirming P3/P4's read path is
unaffected.
