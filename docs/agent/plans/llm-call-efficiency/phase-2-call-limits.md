...
- **Steps:**
  - [ ] Write failing test in `tests/test_config.py`:
    ...
    ```
  - [ ] Verify it fails (`pytest tests/test_config.py -k model_call -v`) — expect FAIL (attributes missing).
  - [ ] Add two fields to `Settings` (near `model_concurrency`):
    ```python
    ...
    ```
  - [ ] Add both vars with their defaults to `scout/.env.example`.
  - [ ] Verify tests pass (`pytest tests/test_config.py -v`).
  - [ ] Commit: `feat(config): add MODEL_MAX_TOKENS and MODEL_TIMEOUT_SECONDS`
...
- **Steps:**
  - [ ] Write failing test in `tests/test_shared_llm.py` (extends the
    ...
    ```
  - [ ] Verify it fails (`pytest tests/test_shared_llm.py::test_complete_json_forwards_max_tokens_and_timeout -v`) — expect FAIL (kwargs absent).
  - [ ] In `complete_json`, pass to `litellm.acompletion`:
        `max_tokens=settings.model_max_tokens,` and
        `timeout=settings.model_timeout_seconds,`.
  - [ ] Verify tests pass (`pytest tests/test_shared_llm.py -v`).
  - [ ] Commit: `feat(llm): cap output tokens and set a request timeout`

---

## Verification

- [ ] Phase tests pass: `pytest tests/test_config.py tests/test_shared_llm.py -v`
