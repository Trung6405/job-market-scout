...
export function searchJobsHandler(params) {
  ...
  try {
    ...
    // The jobspy scraper container is kept running (see docker-compose.yaml,
    // jobspy-scraper service) and each search execs into it, instead of
    // paying a fresh `docker run` container-startup cost on every call.
    const containerName = process.env.JOBSPY_CONTAINER_NAME || 'jobspy-scraper';
    const cmd = `${dockerCmd} exec ${containerName} python main.py ${args.join(' ')}`;
    logger.info(`Executing in container: ${cmd}`);
